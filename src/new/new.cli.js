...
async function newCli(args) {
  ...
  if (nameProject) {
    ...
    if (args['skip-install']) {
      packageJson.dependencies = { '@hectorjs/stub-backend': '1.36.0' };
    }
  ...
  if (nameProject && !args['skip-install']) {
    exec(`${packageManager} ${installCommand} --silent @hectorjs/stub-backend@1.36.0`);
  } else {
    exec(`${packageManager} ${installCommand} --silent @hectorjs/stub-backend@1.36.0 --save-dev`);
  }
